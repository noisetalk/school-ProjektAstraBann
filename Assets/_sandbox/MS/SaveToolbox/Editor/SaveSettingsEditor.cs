...
{
	[CustomEditor(typeof(SaveToolboxPreferences))]
	public class SaveSettingsEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			var saveSettingsSingleton = (SaveToolboxPreferences)serializedObject.targetObject;

			// TODO: Group into default save settings.
			EditorGUILayout.BeginVertical(preferencesBoxStyle);

			GUILayout.Label("Save Toolbox Global Preferences", largeTextStyle);

			// Asynchronous saving?
			saveSettingsSingleton.AsynchronousSaving = EditorGUILayout.Toggle(asynchronousSavingContent, saveSettingsSingleton.AsynchronousSaving);

			// Asynchronous saving settings.
			saveSettingsSingleton.LowestAcceptableLoadingFrameRate = EditorGUILayout.IntField(lowestFrameRateContent, saveSettingsSingleton.LowestAcceptableLoadingFrameRate);

			// Logging.
			saveSettingsSingleton.LoggingEnabled = EditorGUILayout.Toggle(loggingEnabledContent, saveSettingsSingleton.LoggingEnabled);

			EditorGUILayout.EndVertical();

			//TODO: Group into tool preferences.
			EditorGUILayout.BeginVertical(preferencesBoxStyle);

			GUILayout.Label("Default Save Game Settings", largeTextStyle);

			GUILayout.Label("File Settings", mediumTextStyle);

			// Save file name.
			saveSettingsSingleton.DefaultSaveSettings.SaveFileName = EditorGUILayout.TextField(saveFileNameContent, saveSettingsSingleton.SaveFileName);

			// Relative folder path to Application.PersistentDataPath.
			saveSettingsSingleton.DefaultSaveSettings.RelativeFolderPath = EditorGUILayout.TextField(relativeFolderPathContent, saveSettingsSingleton.RelativeFolderPath);

			// Save File Type.
			saveSettingsSingleton.DefaultSaveSettings.SaveFileFormat = (StbFileFormat)EditorGUILayout.EnumPopup(saveFileFormatContent, saveSettingsSingleton.SaveFileFormat);

			// Json Pretty Print.
			if (saveSettingsSingleton.SaveFileFormat == StbFileFormat.Json)
			{
				saveSettingsSingleton.DefaultSaveSettings.JsonPrettyPrint = EditorGUILayout.Toggle(jsonPrettyPrintContent, saveSettingsSingleton.JsonPrettyPrint);
			}

			EditorGUILayout.Space();

			//Serialization SettingsScriptableObject.
			GUILayout.Label("Serialization Settings", mediumTextStyle);

			var fieldSettings = saveSettingsSingleton.SerializationSettings.FieldSerializationSettings;
			...
			var propertySettings = saveSettingsSingleton.SerializationSettings.PropertySerializationSettings;
			...
			// Encryption Type.
			saveSettingsSingleton.StbEncryptionSettings.EncryptionType = (StbEncryptionType)EditorGUILayout.EnumPopup(encryptionTypeContent, saveSettingsSingleton.StbEncryptionSettings.EncryptionType);
			if (saveSettingsSingleton.StbEncryptionSettings.EncryptionType != StbEncryptionType.None)
			{
				// Encryption Keyword.
				saveSettingsSingleton.StbEncryptionSettings.EncryptionKeyword = EditorGUILayout.TextField(encryptionKeywordContent, saveSettingsSingleton.StbEncryptionSettings.EncryptionKeyword);
			}

			// Encryption Type Aes.
			if (saveSettingsSingleton.StbEncryptionSettings.EncryptionType == StbEncryptionType.Aes)
			{
				EditorGUI.BeginDisabledGroup(true);
				// Encryption Initialization Vector for Aes.
				saveSettingsSingleton.StbEncryptionSettings.EncryptionInitializationVector = EditorGUILayout.TextField(initializationVectorContent, saveSettingsSingleton.StbEncryptionSettings.EncryptionInitializationVector);
				EditorGUI.EndDisabledGroup();
				if (GUILayout.Button("Regenerate Initialization Vector"))
				{
					saveSettingsSingleton.DefaultSaveSettings.RegenerateInitializationVector();
				}
			}
			...
			// Compression Type.
			saveSettingsSingleton.DefaultSaveSettings.CompressionType = (StbCompressionType)EditorGUILayout.EnumPopup(compressionTypeContent, saveSettingsSingleton.CompressionType);
			...
			saveSettingsSingleton.DefaultSaveSettings.SaveScene = EditorGUILayout.Toggle(shouldSaveLoadScenesContent, saveSettingsSingleton.SaveScene);

			// Scene Load Type
			if (saveSettingsSingleton.SaveScene)
			{
				saveSettingsSingleton.DefaultSaveSettings.StbSceneSavingType = (StbSceneSavingType)EditorGUILayout.EnumPopup(sceneSaveLoadSolutionContent, saveSettingsSingleton.StbSceneSavingType);
			}

			// Rebuild loadable objects.
			saveSettingsSingleton.DefaultSaveSettings.RebuildLoadableObjects = EditorGUILayout.Toggle(rebuildLoadableObjectsContent, saveSettingsSingleton.RebuildLoadableObjects);

			// Physics sync transform on load.
			saveSettingsSingleton.DefaultSaveSettings.PhysicsSyncTransformsOnLoad = EditorGUILayout.Toggle(syncTransformsContent, saveSettingsSingleton.PhysicsSyncTransformsOnLoad);

			// Freeze time scale when asynchronous saving/loading
			saveSettingsSingleton.DefaultSaveSettings.FreezeTimeScaleOnAsynchronousSaveLoad = EditorGUILayout.Toggle(freezeTimescaleContent, saveSettingsSingleton.FreezeTimeScaleOnSaveLoad);

			EditorGUILayout.EndVertical();

			// Select the loadable object database.
			if (GUILayout.Button("Reset Serialization SettingsScriptableObject"))
			{
				saveSettingsSingleton.DefaultSaveSettings.ResetSerializationSettingsToDefault();
			}

			// Select the loadable object database.
			if (GUILayout.Button("Select Loadable Object Database."))
			{
				Selection.activeObject = LoadableObjectDatabase.Instance;
			}

			// Open save file location.
			if (GUILayout.Button("Open Relative Folder Location"))
			{
				if (string.IsNullOrEmpty(saveSettingsSingleton.RelativeFolderPath))
				{
			...
			{
				var directory = $"{Application.persistentDataPath}/{saveSettingsSingleton.RelativeFolderPath}";
				if (!Directory.Exists(directory))
				{
			...
			//TODO: We should add a button to try open the directory from relative path and folder.
